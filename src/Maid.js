...
// Maid Profile Component
const MaidProfile = ({ name, number, info, photo }) => {
  ...
  return (
    <View style={styles.profileContainer}>
      ...
      <TouchableOpacity style={styles.callButton} onPress={handleCallMaid}>
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.ScrollView contentContainerStyle={styles.contentContainer} style={{ opacity: fadeInOut }}>
        {maidProfiles.map((profile, index) => (
          <MaidProfile key={index} {...profile} />
        ))}
      </Animated.ScrollView>
      <Footer />
    </View>
...
const styles = StyleSheet.create({
  ...
  callButton: {
    backgroundColor: '#2196F3', // Background color of the "Call Maid" button
    padding: 10,
    borderRadius: 5,
    alignItems: 'center',
  },
  callButtonText: {
    color: '#FFFFFF', // Text color of the "Call Maid" button
    fontWeight: 'bold',
  },
});
