...
// Electrician Profile Component
const ElectricianProfile = ({ name, number, info, photo }) => {
  ...
  return (
    <View style={styles.profileContainer}>
      ...
      <TouchableOpacity style={styles.callButton} onPress={handleCallElectrician}>
        <Text style={styles.callButtonText}>Call Electrician</Text>
      </TouchableOpacity>
    </View>
  );
};
...
const Electrician = () => {
  ...
  // Dummy data for electrician profiles
  const electricianProfiles = [
    ...
    // Add more electrician profiles as needed
  ];
  ...
  return (
    <View style={styles.container}>
      ...
      <Animated.ScrollView contentContainerStyle={styles.contentContainer} style={{ opacity: fadeInOut }}>
        {electricianProfiles.map((profile, index) => (
          <ElectricianProfile key={index} {...profile} />
        ))}
      </Animated.ScrollView>
      <Footer />
    </View>
...
const styles = StyleSheet.create({
  ...
  callButton: {
    backgroundColor: '#2196F3', // Background color of the "Call Electrician" button
    padding: 10,
    borderRadius: 5,
    alignItems: 'center',
  },
  callButtonText: {
    color: '#FFFFFF', // Text color of the "Call Electrician" button
    fontWeight: 'bold',
  },
});
